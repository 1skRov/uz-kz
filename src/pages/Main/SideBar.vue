...
<template>
  <div class="side-bar" :class="{ 'side-bar--background': isBackground }">
    <p class="font-gilroy" :style="{ color: isBackground ? '#fff' : '#CFD3DA'}" >{{ page }}</p>
    <svg width="16" height="89" viewBox="0 0 16 89" fill="none" xmlns="http://www.w3.org/2000/svg" v-if="icon">
      <path
        ...
        :style="{ fill: isBackground ? '#fff' : '#CFD3DA' }" />
    </svg>
  </div>
</template>
<style scoped>
